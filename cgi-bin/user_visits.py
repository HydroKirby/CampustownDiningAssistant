...
def prev_rests_with_tag(cursor, username, tag): 
    ...
    if(t_visits >= 3):
        ...
        # Finds all restaurants with the searched tag and joins it
        'INNER JOIN (SELECT restaurants.id FROM restaurants INNER JOIN tags ' ,
        'ON restaurants.id = tags.rest_id WHERE tag_name=%s) b ON ' ,
        # Sort from most visited to least visited, and get the top 5
        'a.rest_id = b.id ORDER BY a.total_visits DESC LIMIT 5'])
        cursor.execute(query, (username, t_visits, tag))
        return cursor
    else:
        return None
